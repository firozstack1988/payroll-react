...
const EditLeavePolicy=()=>{
    ...
    useEffect(()=>{
        axios.get(BASE_URL+"leavePolicy/"+id).then(result=>{
            setLeavePolicy({...leavePolicy,
                sickLeave:result.data[0].sick_leave,
                casualLeave:result.data[0].casual_leave,
                earnedLeave:result.data[0].earned_leave,
    ...
    const handleSubmit=(event)=>{
        ...
        if(Object.keys(validationerror).length===0){
            axios.post(BASE_URL+"leavePolicy/modifyLeavePolicy",leavePolicy)
            .then((result)=>{ 
