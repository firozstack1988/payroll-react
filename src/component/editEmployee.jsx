...
const EditEmployee=()=>{
    ...
    useEffect(()=>{
        axios.get(BASE_URL+"employee/"+id).then(result=>{
            setEmployee({...employee,
                empId:result.data[0].emp_id,
                name:result.data[0].name,
                deptName:result.data[0].dept_name,
                emailAddress:result.data[0].email_address,
                mobileNo:result.data[0].mobile_no,
                designation:result.data[0].emp_designation,
                branchCode:result.data[0].branch_code,
                basicAmt:result.data[0].basic_amt,
                id:result.data[0].id
        }) 
        }).catch((err)=>{
    ...
    const handleSubmit=(event)=>{
        ...
        if(employee.name==''){
            validationerror.name="Employee Name is Required"; 
        }
        if(employee.empId==''){
            validationerror.empId="Employee Id is Required"; 
        }
        ...
        if(Object.keys(validationerror).length===0){
            axios.post(BASE_URL+"employee/modifyEmp",employee)
            .then((result)=>{ 
    ...
    return(    
      ...
      <input className="form-control" type="text" 
       name="empId" value={employee.empId} readonly="true" onChange={handleOnchange} />
